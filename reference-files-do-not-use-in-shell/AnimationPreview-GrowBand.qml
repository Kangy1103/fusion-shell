...
Window {
  ...
  title: "Grow Band — Enter/Space open · Esc close"
  color: "#120f1b"
  ...
  function openIt() { if (box.visible) return; box.visible = true
    ...
    rect.animScale = 0.10
    o_o.from = 0.45; o_o.to = 1; o_o.duration = 180; o_o.easing.type = Easing.Linear
    o_s.from = 0.10; o_s.to = 1.0; o_s.duration = 550; o_s.easing.type = Easing.OutElastic; o_s.easing.amplitude = 0.5; o_s.easing.period = 0.35
  ...
  function closeIt() { if (!box.visible) return
    ...
    c_s.from = 1.0; c_s.to = 0.10; c_s.duration = 200; c_s.easing.type = Easing.InQuad
