...
Window {
  ...
  title: "Gentle Scale+Fade — Enter/Space open · Esc close"
  color: "#120f1b"
  ...
  Text {
    ...
    text: "Gentle Scale+Fade"; color: "#97b6ca"
  ...
  function openIt() { if (box.visible) return; box.visible = true
    ...
    rect.animScale = 0.80
    o_o.from = 0; o_o.to = 1; o_o.duration = 250; o_o.easing.type = Easing.Linear
    o_s.from = 0.80; o_s.to = 1.0; o_s.duration = 350; o_s.easing.type = Easing.OutCubic
  ...
  function closeIt() { if (!box.visible) return
    ...
    c_o.from = 1; c_o.to = 0; c_o.duration = 150; c_o.easing.type = Easing.InQuad
    c_s.from = 1.0; c_s.to = 0.90; c_s.duration = 150; c_s.easing.type = Easing.InQuad
